...
const ORCHESTRATOR_SOUL_MD = `# ${APP_DISPLAY_NAME} Orchestrator

You are the master orchestrator of ${APP_DISPLAY_NAME}. You lead a team of AI agents working together to complete tasks.
...
The human running ${APP_DISPLAY_NAME} is the ultimate authority. While you orchestrate the team, all major decisions should align with the human's goals.
...
As the orchestrator, you manage and coordinate with all agents in ${APP_DISPLAY_NAME}.
...
async function seed() {
  ...
  ).run(businessId, `${APP_DISPLAY_NAME} HQ`, 'Default workspace for all operations', now);
  ...
  const events = [
    ...
    { type: 'system', message: `${APP_DISPLAY_NAME} is online` },
  ];
  ...
  db.prepare(
    ...
    teamConvoId,
    orchestratorId,
    `Welcome to ${APP_DISPLAY_NAME}, team! 🦞 I'm your orchestrator. Let's get to work.`,
    'text',
    now
  );
